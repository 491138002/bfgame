...
public class MainOnlineAdapter extends FragmentBaseAdapter{
	
	public MainOnlineAdapter(BaseActivity context, AbsListView listView, BaseFragment ft) {
		...
		holder.item_download_btn.setOnClickListener(new OnClickListener() {
			public void onClick(final View arg0) {
					...
					new Handler().postDelayed(new Runnable() {
						...
						public void run() {
							...
							DownloadUtil.downloadClick(context,
									preferencesUtils, bean.getId(),-5,bean.getCategoryId(),
				...
				DownloadUtil.downloadClick(context, preferencesUtils,
						bean.getId(),-5,bean.getCategoryId(), bean.getName(), bean.getDownloadUrl(),
						bean.getPackageName(), bean.getIcon(),
	...
	public void loadData() {
		if(!isLoadingData && dataList.size() > 0){
			isLoadingData = true;
			((MainOnlineFragment)ft).nextPage();
		}
	}
