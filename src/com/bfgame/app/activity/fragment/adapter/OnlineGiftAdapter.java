...
import com.bfgame.app.activity.fragment.OnlineGiftFragment.OnGiftResponseListener;
...
public class OnlineGiftAdapter extends FragmentBaseAdapter {
	...
	public OnlineGiftAdapter(BaseActivity context, AbsListView listView,
			BaseFragment ft) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(bean.isGet){
			...
			holder.item_get_btn.setBackgroundResource(R.drawable.bfgame_btn_blue);
		}
		holder.item_get_btn.setOnClickListener(new OnClickListener() {
			public void onClick(View arg0) {
				holder.item_get_btn
						.setText(R.string.bfgame_gift_receive_loading);
				((OnlineGiftFragment)ft).getGiftCode(bean);
				((OnlineGiftFragment)ft).setOnGiftResponseListener(new OnGiftResponseListener() {
					
					@Override
					public void onGet(Gift bean) {
						giftAlertDialog=new GiftAlertDialog(context, bean.getActivationCode());
		...
		for (int i = 0; i < dataList.size(); i++) {
			Gift bean = (Gift) dataList.get(i);
				if(gift.getGiftId().equals(bean.getGiftId())){
					...
					((OnlineGiftFragment)ft).addGift(bean);
			}
		}
	}
